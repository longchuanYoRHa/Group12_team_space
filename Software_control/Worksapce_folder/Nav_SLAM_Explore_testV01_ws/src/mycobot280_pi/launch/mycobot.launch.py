...
def generate_launch_description():
    ...
    # -------------------------------------------------------------------------
    controllers_yaml = os.path.join(pkg_share, "config", "mycobot_280_controllers.yaml")
    with open(urdf_path, "r") as fh:
        robot_description_str = fh.read()
    # Replace config path placeholder
    robot_description_str = robot_description_str.replace("MYCOBOT_CONFIG_PATH", controllers_yaml)
    # Replace package:// URIs with absolute paths for Gazebo
    robot_description_str = robot_description_str.replace("package://mycobot280_pi/", pkg_share + "/")
    ...
    gz_args_text = f"-r -v 4 {world_to_use}"
